...
describe('<PostNavigation />', () => {
  ...
  it('рендерится без ошибок', () => {
    renderWithProviders(<PostNavigation  heads={[{id: "head1", level: 2, text: "Введение"}, {id: "head2", level: 2, text: "Основы"}, {id: "head3", level: 3, text: "Детали"}]} pageTitle="Тестовая статья" markdownBlockRef={{ current: null }} />);
  });
  ...
  it('не отображает навигацию если heads пустой', () => {
    renderWithProviders(<PostNavigation  heads={[]} pageTitle="Тестовая статья" markdownBlockRef={{ current: null }} />);
  ...
  it('не отображает если все heads имеют level 1', () => {
    renderWithProviders(<PostNavigation  heads={[{id: "main", level: 1, text: "Заголовок h1"}]} pageTitle="Тестовая статья" markdownBlockRef={{ current: null }} />);
    expect(screen.queryByRole('navigation')).toBeNull();
  });
