...
describe('<PostMarkdownBlock />', () => {
  ...
  it('рендерится без ошибок', () => {
    renderWithProviders(<PostMarkdownBlock  post="# Заголовок
Текст [ссылка](https://example.com)" markdownBlockRef={{ current: null }} className="custom-markdown-block" />);
  });
  ...
  it('работает с пустым постом', () => {
    renderWithProviders(<PostMarkdownBlock  post="" markdownBlockRef={{ current: null }} className="empty-block" />);
